...
class S2DCIQLImpl(DDPGBaseImpl):
    ...
    def compute_value_loss(self, batch: TorchMiniBatch) -> torch.Tensor:
        """Compute IQL value loss (expectile regression)."""
        with torch.no_grad():
            q_t = self._q_func_forwarder_rs.compute_expected_q(
                batch.observations, batch.actions, "min"
            )
        ...
        diff = q_t - v_t
        weight = (self._expectile - (diff < 0.0).float()).abs()
    ...
    def compute_critic_loss(
        ...
        with torch.no_grad():
            ...
            target_v = self._modules.value_func(batch.next_observations)
        ...
        with torch.no_grad():
            ...
            target_v_squeezed = target_v.squeeze() if target_v.numel() == target_v.shape[0] else target_v.squeeze(-1)
            ...
            td_target = rewards + (self._gamma ** intervals) * target_v_squeezed * (1 - terminals)
    ...
    def inner_update(self, batch: TorchMiniBatch, grad_step: int) -> Dict[str, float]:
        ...
        total_rs_loss = critic_losses["q_loss_rs"] + value_loss
        ...
        critic_losses["q_loss_d"].backward()
        torch.nn.utils.clip_grad_norm_(
            self._modules.q_funcs_d.parameters(),
            self._gradient_clip_norm,
        )
        ...
        # Adjust tau based on KL divergence
